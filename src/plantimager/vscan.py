...
class VirtualScanner(AbstractScanner):
    ...
    def scan(self, path: path.Path, rgb_channel_fileset: Fileset, other_channels_fileset: Fileset) -> None:
        for x in path:
            ...
            for c in self.channels():
                if c == 'rgb':
                    f = rgb_channel_fileset.create_file(data_item.channels[c].format_id())
                else:
                    if other_channels_fileset is not None:
                       f = other_channels_fileset.create_file(data_item.channels[c].format_id())
