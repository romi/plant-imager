...
# Example usages:
###############################################################################
# 1. Default build options will create `roboticsmicrofarms/romiscanner:latest`:
# $ ./build.sh
#
# 2. Build image with 'debug' image tag & another 'romiscanner' branch options:
...
docker build -t roboticsmicrofarms/romiscanner:$vtag $docker_opts \
